 ...
 */
class KeyRecord implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * @var string
     */
    protected static $openAPIModelName = 'KeyRecord';
    ...
    public function listInvalidProperties()
    {
        ...
        if ($this->container['revoked_at'] === null && !$this->isNullableSetToNull('revoked_at')) {
            $invalidProperties[] = "'revoked_at' is required";
        }
        if ($this->container['expires_at'] === null && !$this->isNullableSetToNull('expires_at')) {
            $invalidProperties[] = "'expires_at' is required";
        }
        if ($this->container['created_by_email'] === null && !$this->isNullableSetToNull('created_by_email')) {
            $invalidProperties[] = "'created_by_email' is required";
        }
        ...
        if ($this->container['last4'] === null && !$this->isNullableSetToNull('last4')) {
            $invalidProperties[] = "'last4' is required";
        }
     ...
     * Sets type
     *
     * @param string $type Key kind. `server` (back-end, full SDK auth), `client` (public, browser-embeddable), `admin` (CLI/devtools auth token), `ops` (restricted unattended-trigger credential).
     *
     * @return self
     */
     ...
     * Sets env
     *
     * @param string $env Environment the key is bound to. The worker derives the read env from the key, so this is the isolation boundary. `admin`/`ops` keys are env-agnostic and pinned to `prod`.
     *
     * @return self
     */
     ...
     * Sets created_at
     *
     * @param string $created_at ISO-8601 timestamp the key was minted.
     *
     * @return self
     */
     ...
     * Sets revoked_at
     *
     * @param string|null $revoked_at ISO-8601 timestamp the key was revoked, or `null` while the key is active.
     *
     * @return self
     */
     ...
     * Sets expires_at
     *
     * @param string|null $expires_at ISO-8601 expiry, or `null` for a key that never expires. `admin` keys are fixed at 90 days; `ops` keys use a short sliding window.
     *
     * @return self
     */
     ...
     *
     * @param string|null $created_by_email Email of the human or actor that minted the key, or `null` when not attributable (e.g. CLI device-flow).
     *
     * @return self
     */
     ...
     * Sets name
     *
     * @param string|null $name Human label for the key. `null` for unnamed dashboard mints; programmatic mints auto-name themselves.
     *
     * @return self
     */
     ...
     * Sets description
     *
     * @param string|null $description Free-text note recording what the key is for (the service that holds it, the rotation owner, the ticket it was minted under). Display only — `null` unless someone set it via `PATCH /api/admin/keys/{id}`.
     *
     * @return self
     */
     ...
     * Sets scopes
     *
     * @param string[]|null $scopes Permission strings recorded on the key for audit/display, or `null` when none were set.
     *
     * @return self
     */
     ...
     * Sets last4
     *
     * @param string|null $last4 Last 4 characters of the raw token, so the dashboard mask is matchable against the held key. `null` for keys minted before this column.
     *
     * @return self
     */
