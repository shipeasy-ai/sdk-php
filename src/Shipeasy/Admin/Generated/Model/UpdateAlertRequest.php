...
class UpdateAlertRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets status
     *
     * @param string|null $status New lifecycle state, written through to the queue item (`resolved` / `wont_fix` / `open`). `resolved` / `dismissed` stamp their timestamp; `active` re-opens and clears both.
     *
     * @return self
     */
     ...
     * Sets assignee_id
     *
     * @param string|null $assignee_id PERSON owner — a `users.id`, or `null` to clear the assignment.
     *
     * @return self
     */
     ...
     * Sets agent
     *
     * @param string|null $agent AGENT owner — a connected trigger connector's id (`connectors.id`), the built-in `\"jarvis\"` (Enterprise plan only — rejected with `403` otherwise), or `null` to clear. Stored in `assigneeConnectorId` or `assigneeAgent` depending on the value; the two are mutually exclusive.
     *
     * @return self
     */
